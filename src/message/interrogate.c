 ...
 */
bool secure_send(const uint8_t *const data, const size_t length) {
  if (!channel_is_open) {
    return false;
  } else if ((length == 0) || (length > UINT16_MAX)) {
    return false;
  }
 ...
 */
bool secure_receive(uint8_t *const buffer, const size_t length) {
  if (!channel_is_open) {
    return false;
  } else if ((length == 0) || (length > UINT16_MAX)) {
    return false;
...
void message_interrogate_response(const message_header_t header,
  ...
  if ((header.message_length == 0) ||
      (header.operation != MESSAGE_INTERROGATE) ||
      (header.message_length != sizeof(interrogate_payload_t))) {
    return fail_generic();
  }

  const interrogate_payload_t *const interrogate_payload =
      (const interrogate_payload_t *)payload;

  if (!validate_pin(interrogate_payload->pin)) {
    return fail_generic();
  }

  // TODO: Send initial KEX

  // TODO: Parse KEX response

  // TODO: Send encrypted INT

  // TODO: Parse encrypted INT
